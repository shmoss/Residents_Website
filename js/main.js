...
svg.append("path")
   ...
   //  //queue.js for data loading
    ...
    q		
		.defer(d3.json, "data/world.topojson") //load choropleth spatial data
		.await(callback);

    //once data loaded, callback function
    //takes 4 parameters (including the above three data sources) 
	function callback(error, worldCountry){				
	   
       var countries = topojson.feature(worldCountry, worldCountry.objects.countries).features;
...
function setEnumerationUnits(communityAreas, map, path, colorScale){		
	...
	var country = map.selectAll(".country")
		...
		.attr("class", function(d){ //assign class here
			...
			return "country country_" + d.properties.area_numbe;
		})
		.attr("d", path)
